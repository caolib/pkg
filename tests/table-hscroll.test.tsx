  ...
  try {
    ...
    if (scrollbox) {
      ...
      if (bar) {
        ...
        await act(async () => {
          await setup.mockMouse.drag(Math.round((bar.xStart + bar.xEnd) / 2), barRow, dragTo, barRow)
          await setup.renderOnce()
        })
        check(scrollbox.scrollLeft > startLeft, `拖动滑块横向滚动生效（${startLeft} → ${scrollbox.scrollLeft}）`)
      }
