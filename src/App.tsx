...
type Overlay =
  | { kind: "search" }
  | { kind: "settings" }
  | { kind: "detail"; manager: PackageManager; name: string; managerName: string | null; title: string }
  | { kind: "confirm"; message: string; commands?: string[]; onConfirm: () => void; onCancel: () => void }
...
export function App() {
  ...
  const columns: TableColumn<InstalledRow>[] = isAll
    ? [
        { key: "name", label: t("col.name"), width: 36, render: (r) => r.pkg.display_name || r.pkg.name },
    ...
    : [
        { key: "name", label: t("col.name"), width: 40, render: (r) => r.pkg.display_name || r.pkg.name },
  ...
  async function runUpdate(groups: ManagerGroups) {
    showToast(t("notify.updating", { count: String(Object.values(groups).reduce((a, v) => a + v.length, 0)) }), "info", PROGRESS_TIMEOUT_MS)
    ...
    if (summary.fail === 0) {
      ...
      showToast(t("notify.updated_partial", { ok: String(summary.ok), fail: String(summary.fail) }), "warn", 8000)
    }
  ...
  async function runUninstall(groups: ManagerGroups) {
    const count = Object.values(groups).reduce((a, v) => a + v.length, 0)
    showToast(t("notify.uninstalling", { count: String(count) }), "info", PROGRESS_TIMEOUT_MS)
    ...
    if (summary.fail === 0) {
      ...
      showToast(t("notify.uninstalled_partial", { ok: String(summary.ok), fail: String(summary.fail) }), "warn", 8000)
    }
    setCheckedKeys(new Set())
    reloadAll()
  }
  ...
  function doInstall(mgrName: string, name: string) {
    ...
    ;(async () => {
      showToast(t("notify.installing", { manager: mgrName, name }), "info", PROGRESS_TIMEOUT_MS)
      try {
  ...
  function switchManagerRelative(delta: number) {
    ...
    const seq = [ALL_MANAGERS, ...reg.names.filter((n) => {
