...
export class ManagerRegistry {
  ...
  /** 构造所有已注册管理器的运行时状态。 */
  constructor() {
    for (const [name, cls] of Object.entries(listManagers()).sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  findSelected(current: string, key: string): { manager: PackageManager; state: ManagerState; pkg: PackageInfo } | null {
...
export function buildInstalledRows(reg: ManagerRegistry, opts: InstalledViewOptions): InstalledRow[] {
  ...
  for (const st of managers) {
    ...
    for (const pkg of st.installed) {
      ...
      if (opts.filterText) {
        ...
        if (!shown.includes(opts.filterText) && !pkg.name.toLowerCase().includes(opts.filterText)) continue
      }
      rows.push({
...
export function buildSearchGroups(managers: PackageManager[]): { groups: SearchGroup[]; repMap: Map<string, PackageManager> } {
...
/** 为确认框生成各管理器将执行的命令预览。 */
export function previewCommands(reg: ManagerRegistry, groups: ManagerGroups, action: PreviewAction): string[] {
  const cmds: string[] = []
  for (const [mgrName, names] of Object.entries(groups)) {
    const st = reg.states.get(mgrName)
    if (!st) continue
    cmds.push(action === "update" ? st.instance.updateCommand(names) : st.instance.uninstallCommand(names))
  }
  return cmds
}
...
/** 批量卸载多个管理器的多个包。 */
export async function doUninstallAll(reg: ManagerRegistry, groups: ManagerGroups): Promise<OpSummary> {
  const allResults: OperationResult[] = []
  for (const [mgrName, names] of Object.entries(groups)) {
    const st = reg.states.get(mgrName)
    if (!st) continue
