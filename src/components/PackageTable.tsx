...
export function PackageTable<R>(props: PackageTableProps<R>): ReactNode {
  ...
  const handleScroll = (event: MouseEvent) => {
    ...
    if (event.modifiers.shift) {
      direction =
        direction === "up" ? "left" : direction === "down" ? "right" : direction === "left" ? "down" : "up"
    }
  ...
  const tableInner = (
    <box flexDirection="column" flexGrow={1} onMouseScroll={handleScroll}>
      <box flexDirection="row" columnGap={columnGap}>{headerCells}</box>
      <box flexDirection="row" flexGrow={1}>
        <box flexDirection="column" flexGrow={1} columnGap={columnGap}>{bodyRows}</box>
      </box>
    </box>
  )
