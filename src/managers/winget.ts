...
class WingetPackageManager extends PackageManager {
  ...
  async listOutdated(): Promise<PackageInfo[]> {
    ...
    const rows = _dedupRowsById(_parseTable(stdout, ["id", "name", "version", "available", "source"]))
    const packages: PackageInfo[] = []
    for (const row of rows) {
  ...
  private static _parseShowOutput(stdout: string): Record<string, string> {
    const aliases: Record<string, string> = {
      publisher: "publisher", 发布者: "publisher",
      version: "version", 版本: "version",
      description: "description", 描述: "description",
      homepage: "homepage", 主页: "homepage",
      license: "license", 许可证: "license",
      "package identifier": "id", 包标识符: "id", id: "id",
      name: "name", 名称: "name",
  ...
  updateCommand(packageNames: string[]): string {
    ...
    return packageNames
      .map(
        (n) =>
          `winget upgrade ${n} -e --accept-package-agreements --accept-source-agreements`,
      )
      .join(" && ")
  }
