...
function _splitRowByColumns(
  ...
  const ordered = keys
    .map<[string, number]>((k) => [k, positions[k]])
    .sort((a, b) => a[1] - b[1])
...
class ScoopPackageManager extends PackageManager {
  ...
  async view(packageName: string): Promise<PackageDetail> {
    ...
    if (!data || typeof data !== "object") {
      throw new ManagerError(t("error.view_not_object", { cmd: "scoop cat", package: packageName }))
    }
  ...
  private static _parseManifest(data: Record<string, unknown>, name: string): PackageDetail {
    ...
    if (licenseRaw && typeof licenseRaw === "object") {
      ...
      licenseStr = first && typeof first === "object" ? (first as any).identifier ?? "" : String(first)
    } else {
    ...
    if (Array.isArray(descRaw)) {
      description = descRaw.map((d) => String(d)).join(" ").trim()
    } else {
  ...
  async install(packageName: string): Promise<OperationResult> {
    const { stdout, stderr, exitCode } = await runCommand("scoop", [
      "install",
      packageName,
    ])
