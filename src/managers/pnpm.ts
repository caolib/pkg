...
class PnpmPackageManager extends PackageManager {
  ...
  async listInstalled(): Promise<PackageInfo[]> {
    const { stdout } = await runCommand("pnpm", ["list", "-g", "--json", "--depth", "0"]);
  ...
  async view(packageName: string): Promise<PackageDetail> {
    const { stdout, exitCode } = await runCommand("pnpm", ["info", packageName, "--json"]);
    if (exitCode !== 0) {
    ...
    if (!data || typeof data !== "object") {
      throw new ManagerError(
        t("error.view_not_object", { cmd: "pnpm info", package: packageName }),
      );
    }
  ...
  async install(packageName: string): Promise<OperationResult> {
    const { stdout, stderr, exitCode } = await runCommand("pnpm", ["add", "-g", packageName], {
      log: true,
    });
  ...
  async updateAll(packageNames: string[]): Promise<OperationResult[]> {
    ...
    const { stdout, stderr, exitCode } = await runCommand("pnpm", ["add", "-g", ...specs], {
      log: true,
    });
  ...
  async uninstall(packageName: string): Promise<OperationResult> {
    const { stdout, stderr, exitCode } = await runCommand("pnpm", ["remove", "-g", packageName], {
      log: true,
    });
